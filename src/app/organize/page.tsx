...
import {
  ArrowLeft, ArrowRight, Check, Play, Pause, RotateCcw, Plus, Timer,
  Grid2x2, Columns3, GripVertical, Music, CalendarDays, MapPin, Clock, Flame,
} from "lucide-react";
...
type View = "lanes" | "calendar" | "heatmap" | "matrix" | "focus" | "sounds";
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {t.due && new Date(t.due).getHours() > 0 && (
          <p className="text-xs text-muted">{fmtTime(t.due)}</p>
        )}
      </div>
      <button
        {...attributes}
...
function DropZone({ id, className, children }: { id: string; className?: string; children: React.ReactNode }) {
  ...
  return (
    <div ref={setNodeRef} className={className} style={{ outline: isOver ? "2px solid var(--color-accent)" : undefined, outlineOffset: 3, borderRadius: 14 }}>
      {children}
    </div>
  );
}
...
export default function OrganizePage() {
  ...
  const [showMatrix] = usePref("showMatrix", false);
  ...
  const views: { key: View; label: string; icon: React.ReactNode }[] = [
    ...
    { key: "heatmap", label: "Activity", icon: <Flame size={16} /> },
    ...(showMatrix ? [{ key: "matrix" as View, label: "Matrix", icon: <Grid2x2 size={16} /> }] : []),
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-5 pb-24 pt-6 sm:px-8">
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="glassy flex flex-wrap items-center gap-1 rounded-full border border-line p-1">
          {views.map((v) => (
      ...
      <AnimatePresence mode="wait">
        <motion.div key={view} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }}>
          {view === "sounds" ? (
            <div className="mx-auto max-w-xl"><SoundControls /></div>
          ) : view === "heatmap" ? (
            <Heatmap />
          ) : view === "calendar" ? (
            <Agenda />
      ...
      {(view === "lanes" || view === "matrix" || view === "calendar") && (
        <p className="mt-5 text-center text-sm text-muted">Drag the ⋮ handle to move a task between columns / days.</p>
      )}
    </main>
  );
}
          ...
          const inner = (
            <div className="card h-full p-4">
              ...
              <ul className="space-y-1">
                {items.map((t) => <DragTask key={t.id} t={t} />)}
                {items.length === 0 && <li className="px-2 py-2 text-sm text-muted">—</li>}
              </ul>
            </div>
          );
          return lane.drop ? <DropZone key={lane.key} id={lane.key}>{inner}</DropZone> : <div key={lane.key}>{inner}</div>;
        })}
      </div>
...
/** Multi-day agenda — a week of day columns (calendar + lanes), drag tasks between days. */
function Agenda() {
  const db = useDB();
  ...
  weekStart.setDate(weekStart.getDate() - weekStart.getDay() + weekOffset * 7); // Sunday
  ...
  const onEnd = (e: DragEndEvent) => {
    setDragId(null);
    const id = String(e.active.id);
    const target = e.over?.id as string | undefined; // dayKey
    if (!target) return;
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <button onClick={() => setWeekOffset((o) => o - 1)} className="grid h-9 w-9 place-items-center rounded-full border border-line text-ink-soft hover:text-ink">
          <ArrowLeft size={16} />
        </button>
      ...
      <DndContext sensors={sensors} onDragStart={(e: DragStartEvent) => setDragId(String(e.active.id))} onDragEnd={onEnd} onDragCancel={() => setDragId(null)}>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
          {days.map((d) => {
            ...
            return (
              <DropZone key={k} id={k}>
                <div className="card h-full min-h-[200px] p-3" style={{ borderColor: isToday ? "var(--color-accent)" : undefined }}>
                  <div className="mb-2">
                    <p className="text-xs uppercase tracking-wide text-muted">{d.toLocaleDateString(undefined, { weekday: "short" })}</p>
                    <p className="font-serif text-2xl" style={{ color: isToday ? "var(--color-accent)" : "var(--color-ink)" }}>{d.getDate()}</p>
                  </div>
                  <ul className="space-y-1.5">
                    {dayEvents.map((e) => (
                      <li key={e.id} className="rounded-lg border-l-2 px-2 py-1 text-xs" style={{ borderColor: "var(--color-event)", background: "color-mix(in srgb, var(--color-event) 10%, transparent)", color: "var(--color-event)" }}>
                        <span className="flex items-center gap-1"><Clock size={10} /> {e.allDay ? "All day" : fmtTime(e.start)}</span>
                        {e.title}
                      </li>
                    ))}
                    {dayTasks.map((t) => <DragTask key={t.id} t={t} />)}
                  </ul>
                </div>
              </DropZone>
            );
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[1fr_1.2fr]">
      <div className="card flex flex-col items-center justify-center p-8 text-center">
        ...
        <p className="my-4 font-serif text-8xl tabular-nums text-ink" style={{ color: secs === 0 ? "var(--color-accent)" : undefined }}>{mm}:{ss}</p>
        ...
        <div className="mt-5 flex gap-2 text-sm text-muted">
          {[25, 15, 5, 90].map((m) => (
            <button key={m} onClick={() => { setRunning(false); setSecs(m * 60); }} className="rounded-full border border-line px-3 py-1 transition hover:text-ink">{m}m</button>
          ))}
        </div>
      </div>
